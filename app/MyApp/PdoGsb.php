...
//TP CHGMT MDP //
public function saveNvxMotDePasse($nvxMdp, $dateEmbaucheSaisie ,$idVisiteur){
	//$req = "update visiteur set mdp ='" . $nvxMdp . "' where dateEmbauche ='" . $dateCheckee . "' AND id='" . $idVisiteur . "'";
	$req = "update visiteur set mdp ='$nvxMdp' where dateEmbauche ='$dateEmbaucheSaisie' AND id='$idVisiteur'";
...
*/
	public function getInfosVisiteur($login, $mdp){
		...
		visiteur.dateEmbauche from visiteur 
...
// ajouté pour le TP1
/**
 * Retourne sous forme d'un tableau associatif les id et les prix des frais au forfait
		...
		$req = "select
		 fichefrais.idEtat as idEtat, fichefrais.dateModif as dateModif,
		  fichefrais.nbJustificatifs as nbJustificatifs, 
			fichefrais.montantValide as montantValide, etat.libelle as libEtat from  
			fichefrais inner join etat on fichefrais.idEtat = etat.id 
			where fichefrais.idvisiteur ='$idVisiteur' and fichefrais.mois = '$mois'";
		$res = $this->monPdo->query($req);
		$laLigne = $res->fetch();
		return $laLigne;
